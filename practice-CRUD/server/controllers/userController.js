...
userController.verifyUser = (req, res, next) => {
    ...
    if (!username || !password) return next({ log: 'No username or password in verifyUser in verifyUser.js'});
...
userController.createUser = (req, res, next) => {
    ...
    User.create(
        ...
        (error, user) => {
            if (error) {
                return next('Trouble creating user in userController.js ' + JSON.stringify(error));
            } else {
